...
enum Shell {
    ...
    /// Convenience: run git inside a repo (`git -C <repo> <args...>`). nil if git is
    /// missing. One place so callers don't re-spell the `-C <repo>` prefix.
    static func git(_ repo: String, _ args: [String]) -> Result? {
        run(tool: "git", ["-C", repo] + args)
    }
}
