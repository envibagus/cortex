...
// MARK: - UsageService
//
// Live rate-limit / quota indicator for the local AI coding stack, modeled on
// openusage. All data is REAL and read at the source:
...
final class UsageService {
    ...
    func refresh() async {
        ...
        providers = [
            ...
            ProviderUsage(id: .gemini, result: .notConfigured(
                "The Gemini CLI doesn't expose subscription limits. Not available.")),
        ]
        lastRefresh = Date()
    }
}
...
private enum UsageProbe {
    ...
    /// Pay-as-you-go extra-usage credits -> a dollar-denominated metric.
