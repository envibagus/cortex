...
// Code is the verified default (rich JSON output + cost). Antigravity (Google's agentic
// coding tool, run via the `agy` CLI) and Codex are best-effort text adapters, gated by
// `isInstalled` so only CLIs actually on the machine are selectable.
enum ChatEngine: String, CaseIterable, Identifiable, Hashable {
    ...
    var blurb: String {
        ...
        case .antigravity: "Google's agy CLI"
