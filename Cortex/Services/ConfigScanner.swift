...
final class ConfigScanner {
    ...
    /// Canonical, stable id for a skill/agent file. A plugin skill is installed twice on
    /// disk - a versioned `cache` copy and a `marketplaces` copy - and the same plugin is
    /// often published in more than one marketplace. All of these are the SAME skill, so
    /// reduce every plugin-skill path to one `<plugin>/<skill>` identity (dropping the
    /// volatile version dir AND the marketplace/owner name) so the copies dedupe to a
    /// single entry. Non-plugin skills fall back to their resolved filesystem path.
    private nonisolated static func canonicalResolvedID(for fileURL: URL) -> String {
        if let key = pluginSkillKey(fileURL.path) { return "claude-plugin:\(key)" }
        return fileURL.resolvingSymlinksInPath().path
    }

    /// Extract a stable `<plugin>/<skill>` key from a Claude Code plugin-skill path,
    /// identical across all on-disk layouts so the copies of one installed skill dedupe:
    ///   cache:       .../.claude/plugins/cache/<owner>/<plugin>/<version>/skills/<skill>/SKILL.md
    ///   marketplace: .../.claude/plugins/marketplaces/<owner>/plugins/<plugin>/skills/<skill>/SKILL.md
    ///   loose mkt:   .../.claude/plugins/marketplaces/<owner>/<skill>/SKILL.md   (no "skills" dir)
    /// The `<skill>` is always SKILL.md's parent folder. The `<plugin>` is the segment
    /// right after <owner> in cache, the segment right before "skills" in a standard
    /// marketplace, or <owner> itself in the loose layout. The volatile version dir and
    /// the marketplace/owner name are deliberately dropped so the same plugin published in
    /// more than one marketplace (or cached + in a marketplace) collapses to one entry;
    /// the plugin folder is KEPT so two different plugins that each ship a generically
    /// named skill (e.g. discord/configure vs telegram/configure) stay distinct.
    /// Returns nil when the path is not a recognizable plugin skill.
    private nonisolated static func pluginSkillKey(_ path: String) -> String? {
        // The skill is always the segment right AFTER "skills" (not SKILL.md's immediate
        // parent, which differs when the skill nests its SKILL.md in a subfolder like
        // .../skills/<skill>/upstream/SKILL.md - keying off the parent would collapse every
        // such skill to one id and silently drop all but the first).
        // cache: <owner>/<plugin>/<version?>/skills/<skill>/...  -> plugin = parts[1]
        if let range = path.range(of: ".claude/plugins/cache/") {
            let parts = String(path[range.upperBound...]).components(separatedBy: "/")
            guard parts.count >= 2, let s = parts.firstIndex(of: "skills"), s + 1 < parts.count else { return nil }
            return "\(parts[1])/\(parts[s + 1])"
        }
        // marketplace: <owner>/plugins/<plugin>/skills/<skill>/...  (standard, plugin = the
        //              segment before "skills") or <owner>/<skill>/...  (loose, no "skills" dir).
        if let range = path.range(of: ".claude/plugins/marketplaces/") {
            let parts = String(path[range.upperBound...]).components(separatedBy: "/")
            guard parts.count >= 2, parts.last == "SKILL.md" else { return nil }
            if let s = parts.firstIndex(of: "skills"), s > 0, s + 1 < parts.count {
                return "\(parts[s - 1])/\(parts[s + 1])"
            }
            return "\(parts[0])/\(parts[parts.count - 2])"  // loose: <owner>/<skill>
        }
        return nil
    }
