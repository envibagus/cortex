...
struct ProjectLibraryRollup: Identifiable, Sendable, Hashable {
    ...
    var mcp: Int
    var memory: Int
    // Newest modified date across this project's config items + memory, for the
    // "Last updated" sort. .distantPast when nothing has a date.
    var lastUpdated: Date = .distantPast
    // The actual item names per category, so the Library table's per-category dropdowns
    // can list them and filter projects by membership (the counts above are their sizes).
    var skillNames: [String] = []
    var agentNames: [String] = []
    var commandNames: [String] = []
    var mcpNames: [String] = []
    var memoryNames: [String] = []
...
struct ProjectTechRollup: Identifiable, Sendable, Hashable {
    ...
    // The repo's last commit date, for the "Last updated" sort (nil when unknown).
    var lastUpdated: Date? = nil
}

// MARK: - Project table sort
//
// The sort options for the home Library / Stack "By project" tables. "Last created"
// is intentionally absent: the scanned data carries a modified date per item but no
// creation date, so a "created" sort would be guesswork.

enum ProjectSort: String, CaseIterable, Identifiable, FilterSortOption {
...
enum ProjectInsights {
    ...
    static func libraryRollups(config: ConfigScanner) -> [ProjectLibraryRollup] {
        ...
        // The item names per category for a project (counts are their sizes).
        func itemNames(_ kind: ConfigKind, _ project: String) -> [String] {
            config.items
                .filter { !$0.isGlobal && $0.projectName == project && $0.kind == kind }
                .map(\.name)
        }
        func mcpNames(_ project: String) -> [String] {
            config.mcpServers.filter { $0.scope == project }.map(\.name)
        }
        func memoryNames(_ project: String) -> [String] {
            config.memories.filter { $0.scope == project }.map(\.name)
        }

        // Newest modified date across this project's config items + memory files.
        func lastUpdated(_ project: String) -> Date {
            var d = Date.distantPast
            for item in config.items where !item.isGlobal && item.projectName == project { d = max(d, item.modified) }
            for mem in config.memories where mem.scope == project { d = max(d, mem.modified) }
            return d
        }

        let rollups = names.map { project -> ProjectLibraryRollup in
            let sk = itemNames(.skill, project)
            let ag = itemNames(.agent, project)
            let cm = itemNames(.command, project)
            let mc = mcpNames(project)
            let me = memoryNames(project)
            return ProjectLibraryRollup(
                projectName: project,
                skills: sk.count,
                agents: ag.count,
                commands: cm.count,
                mcp: mc.count,
                memory: me.count,
                lastUpdated: lastUpdated(project),
                skillNames: sk, agentNames: ag, commandNames: cm, mcpNames: mc, memoryNames: me
            )
        }
    ...
    static func techRollups(repos: [RepoInfo]) async -> [ProjectTechRollup] {
        let projects = repos.map { (name: $0.name, path: $0.path, lastCommit: $0.lastCommit) }
        return await Task.detached(priority: .utility) {
            projects
                .compactMap { scanProject(name: $0.name, path: $0.path, lastCommit: $0.lastCommit) }
    ...
    private nonisolated static func scanProject(name: String, path: String, lastCommit: Date?) -> ProjectTechRollup? {
