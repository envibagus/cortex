...
actor BrandIconLoader {
    ...
    // Inline fallbacks for slugs whose CDN icon uses SVG features SwiftDraw can't render
    // (filters / alpha masks). We ship the logo's silhouette path, which renders cleanly
    // as the monochrome template every brand mark already uses. google-antigravity's
    // `default.svg` is built from blurred color blobs clipped to this shape.
    private let builtin: [String: String] = [
        "google-antigravity": #"<svg width="16" height="15" viewBox="0 0 16 15" xmlns="http://www.w3.org/2000/svg"><path d="M14.0777 13.984C14.945 14.6345 16.2458 14.2008 15.0533 13.0084C11.476 9.53949 12.2349 0 7.79033 0C3.34579 0 4.10461 9.53949 0.527295 13.0084C-0.773543 14.3092 0.635692 14.6345 1.50293 13.984C4.86344 11.7076 4.64663 7.69664 7.79033 7.69664C10.934 7.69664 10.7172 11.7076 14.0777 13.984Z" fill="black"/></svg>"#,
    ]
    ...
    func data(for slug: String) async -> Data? {
        ...
        // Inline override wins over the CDN for icons SwiftDraw can't render remotely.
        if let inline = builtin[trimmed] { return Data(inline.utf8) }
        ...
        // silhouette that tints well as a template), then color, then default (some
        // icons ship only `default.svg`, e.g. google-antigravity). All render as a
        // tinted template regardless of the source variant's colors.
        for variant in ["mono", "color", "default"] {
