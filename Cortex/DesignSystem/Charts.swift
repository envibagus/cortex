...
struct HorizontalBars: View {
    ...
    // Drives the fill animation: each bar grows from 0 to its width on appear.
                ...
                HStack(spacing: 10) {
                    ...
                    GeometryReader { geo in
                        ZStack(alignment: .leading) {
                            ...
                            RoundedRectangle(cornerRadius: 4)
                                .fill(row.tint)
                                .frame(width: filled ? max(6, geo.size.width * (row.value / maxValue)) : 0)
                        }
                    }
                    .frame(height: 14)
                    Text(row.valueText)
...
struct ContributionHeatmap: View {
    ...
    var body: some View {
        ...
        return VStack(alignment: .leading, spacing: 8) {
            ...
            // Responsive 7-row grid that fills the card width: a left gutter holds the
            // weekday labels (Mon/Wed/Fri), a month-abbreviation row sits on top aligned to
            // the columns (GitHub-style), and as many of the most-recent week columns as fit
            // are pinned flush-RIGHT so the latest activity stays at the right edge.
            GeometryReader { geo in
                let avail = geo.size.width - gutter
                let fit = max(1, Int((avail + gap) / (cellSize + gap)))
                let visible = Array(allWeeks.suffix(fit))
                VStack(alignment: .trailing, spacing: 4) {
                    monthRow(visible)
                    HStack(alignment: .top, spacing: gap) {
                        weekdayLabels.frame(width: gutter - gap, alignment: .trailing)
                        grid(visible)
                    }
                }
                .frame(maxWidth: .infinity, alignment: .trailing)
            }
            .frame(height: monthRowHeight + 4 + gridHeight)
        }
    }

    /// Left gutter width reserved for the weekday labels.
    private var gutter: CGFloat { 30 }
    private var monthRowHeight: CGFloat { 13 }

    /// Mon / Wed / Fri labels down the left, aligned to the grid's weekday rows (GitHub
    /// shows only the alternating weekdays). Sunday is row 0.
    ...
    /// Month abbreviations across the top, each placed above the first visible column of its
    /// month. Uses the same per-column widths + gutter as the grid so labels line up; the
    /// label is allowed to overflow its column to the right (months are weeks apart).
    private func monthRow(_ visible: [[HeatCell?]]) -> some View {
        HStack(spacing: gap) {
            Color.clear.frame(width: gutter - gap, height: monthRowHeight)
            ForEach(Array(visible.enumerated()), id: \.offset) { i, _ in
                Text(monthLabel(at: i, in: visible))
                    .font(.system(size: 10))
                    .foregroundStyle(Theme.textTertiary)
                    .fixedSize()
    ...
    /// The short month name when column `i` is the first of its month, else "".
    private func monthLabel(at i: Int, in visible: [[HeatCell?]]) -> String {
        let cal = Calendar.current
        guard let date = firstDate(visible[i]) else { return "" }
        let month = cal.component(.month, from: date)
        if i > 0, let prev = firstDate(visible[i - 1]), cal.component(.month, from: prev) == month {
            return ""
        }
        return cal.shortMonthSymbols[month - 1]
    }

    /// The first real (non-padding) day in a week column.
    private func firstDate(_ week: [HeatCell?]) -> Date? {
        week.compactMap { $0?.date }.first
    }

    /// The week-column grid for the given (already weekday-aligned) weeks.
    ///
    /// Hover is tracked with a SINGLE `.onContinuousHover` over the whole grid (the
    /// pointer's local point is mapped to a column/row), not a per-cell `.onHover` +
    /// `.help`. Hundreds of per-cell tracking + tooltip regions made the enclosing
    /// scroll view stutter; one region keeps the highlight + floating caption cheap.
    private func grid(_ weeks: [[HeatCell?]]) -> some View {
        ...
        .onContinuousHover { phase in
            switch phase {
            case .active(let point):
                // Map the local point to a column (week) + row (weekday). The gap maps
                // to the following cell, which is fine for a highlight.
                let col = Int(point.x / (cellSize + gap))
                let row = Int(point.y / (cellSize + gap))
                if weeks.indices.contains(col), (0..<7).contains(row), let cell = weeks[col][row] {
                    hovered = cell
                } else {
                    hovered = nil
                }
    ...
    /// padding days that keep the weekday rows aligned. Hover is handled by the grid's
    /// single tracker above, so the tile only reacts to the shared `hovered` state.
    @ViewBuilder
