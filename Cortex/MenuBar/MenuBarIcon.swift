...
enum MenuBarIcon {
    ...
    // MARK: - Text-mode title image
    //
    // Renders the text-mode content (an optional leading SF Symbol glyph or a fixed-color
    // dot, then a series of text runs) into ONE status-item image. When every element is
    // adaptive (no fixed accent color), the image is a TEMPLATE so AppKit tints it to
    // contrast the ACTUAL menu bar background - legible over a white wallpaper, a dark one,
    // or when the item is highlighted, and re-tinted automatically when the bar changes.
    // (A plain attributedTitle with .labelColor only tracks the app's light/dark MODE, so it
    // stays white-on-white over a light bar.) When a fixed accent is present (orange warning,
    // green Done, yellow awaiting dot) it renders non-template in those colors, which read on
    // both light and dark bars.

    /// One run of text in the menu-bar title. `color == nil` means "adaptive": it follows
    /// the bar via template tinting (or the label color in the accent case).
    struct MenuTitleRun {
        let text: String
        let color: NSColor?
    ...
    static func textImage(leadingGlyph: String?, leadingDot: NSColor?,
                          runs: [MenuTitleRun], appearance: NSAppearance) -> NSImage {
        func font(_ w: NSFont.Weight) -> NSFont { .systemFont(ofSize: 13, weight: w) }

        // Template only when nothing carries a fixed accent color.
        let isTemplate = leadingDot == nil && runs.allSatisfy { $0.color == nil }
        let adaptiveColor: NSColor = isTemplate ? .black : .labelColor

        let measured: [(text: NSString, attrs: [NSAttributedString.Key: Any], size: NSSize)] = runs.map {
            let attrs: [NSAttributedString.Key: Any] = [.font: font($0.weight),
                                                        .foregroundColor: $0.color ?? adaptiveColor]
            let ns = $0.text as NSString
            return (ns, attrs, ns.size(withAttributes: attrs))
        }

        let glyphImg = leadingGlyph.flatMap { glyph($0, pointSize: 12) }
        let glyphSize = glyphImg?.size ?? .zero
        let dot: CGFloat = leadingDot != nil ? 9 : 0
        let leadingW = max(glyphSize.width, dot)
        let gap: CGFloat = leadingW > 0 ? 3 : 0

        let textW = measured.reduce(0) { $0 + ceil($1.size.width) }
        let height = ceil(max(measured.map { $0.size.height }.max() ?? 15, glyphSize.height, dot, 15))
        let width = ceil(leadingW + gap + textW)

        // Render through a drawingHandler (not lockFocus) so AppKit rasterizes at the
        // display's backing scale - crisp on Retina, matching native menu bar text instead
        // of a blurry up-scaled 1x bitmap.
        let image = NSImage(size: NSSize(width: max(width, 1), height: height), flipped: false) { _ in
            func draw() {
                ...
                if let glyphImg {
                    ...
                    rect.fill(using: .sourceAtop)
                    x = leadingW + gap
                } else if let leadingDot {
                    let rect = NSRect(x: 0, y: (height - dot) / 2, width: dot, height: dot)
                    leadingDot.setFill()
                    NSBezierPath(ovalIn: rect).fill()
                    x = leadingW + gap
                }
                for m in measured {
                    m.text.draw(at: NSPoint(x: x, y: (height - m.size.height) / 2), withAttributes: m.attrs)
                    x += ceil(m.size.width)
                }
            }
            if isTemplate { draw() } else { appearance.performAsCurrentDrawingAppearance { draw() } }
            return true
        }
