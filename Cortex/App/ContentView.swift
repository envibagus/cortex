...
struct ContentView: View {
    ...
    // Captured so the menu bar can reopen this window after it's been closed (the
    // closure stays valid even when no window is open).
    ...
    private var mainShell: some View {
        NavigationSplitView(columnVisibility: $columnVisibility) {
            ...
            // Each route change cross-fades + scales the new page in. The ZStack lets the
            // outgoing + incoming pages overlap during the transition, and .animation on
            // the ZStack (the closest ancestor) reliably drives the .id-change transition.
            ZStack {
            ...
            .background(Theme.canvas)
                ...
                .onChange(of: model.route) { _, _ in
                    // Defer the stale-refresh until after the page transition settles, so a
                    // mid-animation data reload can't stutter the incoming page.
                    Task {
        ...
        // ⌘\ toggles the sidebar column. Toggle WITHOUT animation: animating the column
        // re-lays-out the (heavy, table-backed) detail every frame, which stutters; an
        // instant collapse/expand is a single layout pass, so it's crisp.
        ...
        // NOTE: the live-usage re-probe and the running-`claude` scan loops live on
        // AppModel (startBackgroundLoops), not here, so the menu bar stays fresh even
        // when the main window is closed (this .task would be torn down with the window).
    }
}
