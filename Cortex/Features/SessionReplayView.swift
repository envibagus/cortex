...
struct SessionReplayView: View {
    ...
    // The pointer's Y within the minimap while hovering, used to float a time tooltip
    // at that row. nil when not hovering.
    ...
    // Transcript search: query + which match the playhead is parked on (0-based). Jumping
    // a match just moves `currentID`, so the feed scrolls + highlights via existing wiring.
    ...
    private var content: some View {
        if let loadError {
            ...
            if events.isEmpty {
                ...
                VStack(spacing: 0) {
                    // Top bar: search the transcript (with match count + prev/next) on the
                    // left, the oldest/newest order toggle on the right.
                    HStack(spacing: 10) {
                        ReplaySearchField(
                            query: $searchQuery,
                            matchCount: searchMatches.count,
                            cursor: matchCursor,
                            onPrev: { jumpToMatch(matchCursor - 1) },
                    ...
                    // Transcript feed + color-coded minimap/scrubber.
                    HStack(spacing: 0) {
                        ReplayTranscript(events: events, currentID: $currentID, reversed: reversed)
                        Divider().overlay(Theme.stroke)
                        ReplayMinimap(events: events, currentID: $currentID, hoverID: $hoverID,
                                      hoverY: $hoverY, reversed: reversed)
                    }
                    // Floating time tooltip: while hovering the minimap, show the clock
                    // time of the event under the pointer, parked just left of the strip
                    // at the pointer's row.
                    .overlay(alignment: .topTrailing) {
                        if let hid = hoverID, let hy = hoverY,
                           hid >= 0, hid < events.count, let ts = events[hid].timestamp {
                            Text(Fmt.clockTime(ts))
                ...
                // Re-run the search whenever the query changes: jump to the first match.
                .onChange(of: searchQuery) { _, _ in matchCursor = 0; jumpToMatch(0) }
            }
        } else {
    ...
    // Event ids whose text matches the search query (>= 2 chars). event.id == position in
    // the full feed, which is exactly what currentID / scrollPosition expect.
    private var searchMatches: [Int] {
        let q = searchQuery.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        guard q.count >= 2, let events else { return [] }
        return events.filter {
            $0.body.lowercased().contains(q) || ($0.title?.lowercased().contains(q) ?? false)
        }.map(\.id)
    }

    // Move the playhead to the match at `cursor` (wrapping), which scrolls + highlights it.
    private func jumpToMatch(_ cursor: Int) {
        let matches = searchMatches
        guard !matches.isEmpty else { return }
        let c = ((cursor % matches.count) + matches.count) % matches.count
        matchCursor = c
        isPlaying = false
        withAnimation(.easeInOut(duration: 0.25)) { currentID = matches[c] }
    }
...
private struct ReplayBubble: View {
    ...
    var body: some View {
        // No leading accent stripe: the role icon/label in the header and the focused
        // outline already convey kind + playhead, so the bar was redundant chrome.
        ...
        GeometryReader { geo in
            ...
            .onContinuousHover { phase in
                ...
                case .active(let point):
                    hoverID = index(forY: point.y, height: h, count: n)
                    hoverY = point.y
                case .ended:
                    hoverID = nil
                    hoverY = nil
...
// MARK: - Transcript search field
//
// A compact search box for the replay top bar: type to find matching messages, then
// step through them with the count + up/down chevrons (or Return). Matching just moves
// the playhead, so the feed scrolls to and highlights each hit.

private struct ReplaySearchField: View {
    @Binding var query: String
    let matchCount: Int
    let cursor: Int               // 0-based index of the active match
    let onPrev: () -> Void
    let onNext: () -> Void
...
private struct ReplayOrderToggle: View {
    ...
    var body: some View {
        Button { withAnimation(.easeInOut(duration: 0.18)) { reversed.toggle() } } label: {
            // Same font + vertical padding as ReplaySearchField so both top-bar controls
    ...
    // Liquid Glass on macOS 26, a card fill + hairline below it.
    @ViewBuilder
    private var glassBackground: some View {
        if #available(macOS 26.0, *) {
            Color.clear.glassEffect(.regular, in: RoundedRectangle(cornerRadius: 8, style: .continuous))
        } else {
