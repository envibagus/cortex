...
struct CostsView: View {
    ...
    // Drives the "how costs are calculated" popover on the title (same explainer as
    // the Home Est. Cost tile).
...
// MARK: - Per-model breakdown card (model, tokens, share, cost)
//
// Uses the app's shared InsightTable (the same table component the other breakdowns
// use), plus a totals footer aligned under the table columns.

private struct ModelBreakdownCard: View {
    let stats: UsageStats

    // Shared column widths so the totals footer lines up under the table columns.
    private let tokensW: CGFloat = 90
    private let shareW: CGFloat = 56
    private let costW: CGFloat = 92
        ...
        GroupBox {
            VStack(alignment: .leading, spacing: 12) {
                ...
                if models.isEmpty {
                    ...
                    // Shared InsightTable: a colored model dot + name, then tokens, share, cost.
                    InsightTable(columns: [
                        InsightColumn(title: "Model", width: nil),
                        InsightColumn(title: "Tokens", width: tokensW),
                        InsightColumn(title: "Share", width: shareW),
                        InsightColumn(title: "Cost", width: costW),
                    ], rows: models) { m, index in
                        switch index {
                        case 0:
                            HStack(spacing: 8) {
                                Circle().fill(m.tint).frame(width: 8, height: 8)
                    ...
                    // Totals footer, aligned under the table columns.
                    HStack(spacing: 12) {
                        Text("Total").fontWeight(.bold).foregroundStyle(.primary)
                            .frame(maxWidth: .infinity, alignment: .leading)
                        Text(Fmt.compact(totalTokens))
                            .font(.callout.monospacedDigit().bold()).foregroundStyle(.secondary)
                            .frame(width: tokensW, alignment: .trailing)
                        Color.clear.frame(width: shareW, height: 1)
                        Text(Fmt.money(totalCost))
                            .font(.callout.monospacedDigit().bold()).foregroundStyle(Theme.yellow)
