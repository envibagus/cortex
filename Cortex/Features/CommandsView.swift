...
// MARK: - CommandsView
//
// Browses every command discovered by ConfigScanner (~/.claude/commands and the
// per-project equivalents). It reuses the shared ConfigBrowser defined in
...
// bar are identical to Skills, parameterized by the .command ConfigKind and a purple
...
struct CommandsView: View {
    ...
    var body: some View {
        ConfigBrowser(
            ...
            emptyMessage: "Add commands under ~/.claude/commands to see them here.",
            items: model.config.commands
        )
    }
}
