...
struct UsageMiniRow: View {
    ...
    // Drives the fill animation: the bar grows from 0 to its value when the row appears.
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            // Track + fill (fill reflects consumption, so the bar grows as you use). The
            // fill animates from 0 to its width when the row appears.
