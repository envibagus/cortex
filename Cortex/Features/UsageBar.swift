    ...
    private func body(for result: UsageResult) -> some View {
        ...
        case let .ok(_, metrics):
            ...
            if session == nil && weekly == nil {
                ...
                VStack(spacing: 12) {
                    if let session { UsageMiniRow(metric: session, accent: Theme.yellow) }
                    if let weekly { UsageMiniRow(metric: weekly, accent: Theme.green) }
                }
            }
...
// MARK: - One limit window (label + dot, track + fill, percent + reset)

private struct UsageMiniRow: View {
