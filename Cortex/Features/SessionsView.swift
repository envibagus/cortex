...
private struct SessionDetail: View {
            ...
            case .summary:
                ScrollView {
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        // Details sit right under the summary (primary model lives here, so
                        // there's no separate "Models used" card).
                        GroupBox { metaTable }

                        // Messages + token usage side by side. fillHeight makes the shorter
                        // card match the taller one's height so their borders line up.
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            VStack(spacing: 5) {
                compactRow("Input", Fmt.grouped(session.usage.input),
                           help: "Fresh tokens sent to the model this session (your prompts + files), not counting cached context.")
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            VStack(spacing: 5) {
                compactRow("Cost", Fmt.money(session.cost), valueColor: Theme.yellow, bold: true,
                           help: "Estimated from this session's token counts times each model's per-token price. An estimate, not an official bill.")
                compactRow("Duration", durationText,
                           help: "Time between the first and last recorded message - idle gaps included.")
                compactRow("Started", session.startedAt.formatted(date: .abbreviated, time: .shortened))
                compactRow("Last active", session.endedAt.formatted(date: .abbreviated, time: .shortened),
                           help: "When the last message was written. A session can be idle (not ended) and resume later.")
                if let primary = session.primaryModel {
                    compactRow("Primary model", CostService.displayName(primary))
                }
                if let entry = session.entrypointLabel {
                    compactRow("Started via", entry)
                }
                if let version = session.version, !version.isEmpty {
                    compactRow("Claude Code", "v\(version)")
                }
                if let slug = session.slug, !slug.isEmpty {
                    compactRow("Session name", slug)
                }
    ...
    // One dense key/value line (no divider) for the compact tables. An optional `help`
    // string adds a hover tooltip explaining the figure (e.g. how cost is derived).
    private func compactRow(_ label: String, _ value: String,
                            valueColor: Color = .primary, bold: Bool = false,
...
private struct DetailHeader: View {
    ...
    // True when this exact session id is currently mid-turn per the activity hooks, so
    // we say "active now" instead of the misleading "ended" (the process may be idle,
    // not finished). Falls back to "last active <relative>" otherwise.
    private var isLive: Bool {
        model.activity.activeSessions.contains { $0.session == session.id }
    }
            ...
            HStack(alignment: .center, spacing: 14) {
                VStack(alignment: .leading, spacing: 4) {
                    Text(session.projectName)
                        ...
                        .foregroundStyle(.primary)
                        .lineLimit(2)
                    // AI-generated session title, when Claude Code recorded one.
                    ...
                    // Codename + entrypoint chips, then the relative end time.
                    HStack(spacing: 8) {
                        if let slug = session.slug, !slug.isEmpty {
                            Pill(text: slug, tint: Theme.claude)
                        }
                        if let entry = session.entrypointLabel {
                            Pill(text: entry, tint: Theme.blue)
                        }
                        Text(isLive ? "active now" : "last active \(Fmt.relative(session.endedAt))")
                            .font(.callout)
                            .foregroundStyle(isLive ? Theme.green : .secondary)
                    }
                }
                Spacer(minLength: 0)
            }
